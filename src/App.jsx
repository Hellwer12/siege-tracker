...
/* ══════════════════════════════════════════════════════════════════════════
   i18n — FR / EN  (défini avant tout usage de t())
══════════════════════════════════════════════════════════════════════════ */
let CURRENT_LANG="fr";
const LANG={
  fr:{
    counterpick:"Contre-pick", analyse:"Analyse", guilde:"Guilde", combat:"Détail",
    cpSub:"Défense adverse → meilleures offenses à jouer",
    cpLive:"Live ·",
    defMode:"Défense complète", monMode:"Par monstre",
    wrMin:"WR≥", attMin:"Att≥",
    ...
    import:"↑ Import",
    topOffTitle:"Top Offenses de la Guilde",
    topDefTitle:"Top Défenses rencontrées",
    worstTitle:"Défenses qui nous battent",
    worstSub:"pondéré volume+taux · min 3 att · clic → offenses gagnantes",
    worstOff:"Offenses gagnantes contre cette défense",
    noVictory:"Aucune victoire enregistrée",
    members:"Membres", rivalries:"Rivalités",
    searchPlayer:"Rechercher un pseudo…",
    searchGuild:"Rechercher une guilde…",
    noGuild:"Aucune guilde trouvée",
    defsUsed:"Défenses les plus utilisées",
    hardDefs:"Défenses difficiles",
    offenses:"Offenses",
    detailTitle:"Détail combat",
    ...
    reduce:"▲ Réduire", more:"▾",
    axeVariant:"Axe de réflexion",
  },
  en:{
    counterpick:"Counter-pick", analyse:"Analysis", guilde:"Guild", combat:"Log",
    cpSub:"Enemy defense → best offenses to play",
    cpLive:"Live ·",
    defMode:"Full defense", monMode:"By monster",
    wrMin:"WR≥", attMin:"Att≥",
    ...
    noResult:"No results — try partial name, lower WR min or Att min",
    ...
    reduce:"▲ Collapse", more:"▾",
    axeVariant:"Line of thought",
  },
};
function t(key){ return LANG[CURRENT_LANG]?.[key]||key; }

/* ══════════════════════════════════════════════════════════════════════════
   MONSTER CACHE — Supabase table monsters
══════════════════════════════════════════════════════════════════════════ */
...
async function loadMonsterCache(){
  ...
  try{
    ...
    if(data) data.forEach(m=>MONSTER_CACHE.set(m.name,{image_url:m.image_url,element:m.element||null}));
    MONSTER_CACHE_LOADED=true;
  }catch(e){ console.warn("Monster cache:",e.message); }
}

function monsterImg(name){ return MONSTER_CACHE.get(name)?.image_url||null; }
function monsterElement(name){ return MONSTER_CACHE.get(name)?.element||null; }

const ELEM_ICON={Fire:"🔥",Water:"💧",Wind:"💨",Light:"⭐",Dark:"🌑"};
const COUNTER_ELEM={Fire:"Water",Water:"Wind",Wind:"Fire"};
const SAME_ELEM_GROUP=[["Fire"],["Water"],["Wind"],["Light","Dark"]];
...
function MonsterChip({name,size=22}){
  ...
  return(
    <span style={{display:"inline-flex",alignItems:"center",gap:4,
      background:T.s3,borderRadius:6,padding:"2px 7px 2px 3px",fontSize:11,color:T.ink2}}>
      {img
...
/* ══════════════════════════════════════════════════════════════════════════
   COMPOUND NAMES — 269 noms composés Swarfarm, du plus long au plus court
   Utilisés par extractMonsterNames pour ne pas splitter "Feng Yan", "Mo Long"…
══════════════════════════════════════════════════════════════════════════ */
const COMPOUND_NAMES=[
  "Imperfect Dokkaebi Princess and Sapsaree",
  "Legion Commander of Retribution",
  "Frostjade Beast Khi\'zar Kha\'jul",
  "Legion Commander of Conviction",
  "Dokkaebi Princess and Sapsaree",
  "Blazing Beast Khi\'zar Kha\'jul",
  "Tempest Beast Khi\'zar Kha\'jul",
  "Legion Commander of Judgment",
  "Attack Bar Decrease Crystal",
  "Attack Bar Recharge Crystal",
  "Chaos Beast Khi\'zar Kha\'jul",
  "Spires of Ascension Crystal",
  "Crit Rate Decrease Crystal",
  "Abyssal Ancient Lich King",
  "The Witch-king of Angmar",
  "White Tiger Blade Master",
  "Imperfect Kung Fu Girl",
  "Dimensional Minion (Water)",
  "Strengthen Defense Crystal",
  "Enraged Dimension Predator",
  "Dimensional Minion (Wind)",
  "Strengthen Attack Crystal",
  "Incarnation of Protection",
  "Imperfect Nine-tailed Fox",
  "Dimensional Minion (Fire)",
  "Abyssal Ancient Tormentor",
  "Abyssal Legendary Dragon",
  "Abyssal Ancient Punisher",
  "Disturb Recovery Crystal",
  "Imperfect Chakram Dancer",
  "Magic Order Elementalist",
  "Imperfect Neostone Agent",
  "Imperfect Centaur Knight",
  ...
  "Dessert Kingdom Dragon",
  "Azure Dragon Swordsman",
  "Abyssal Ancient Spirit",
  "Vermilion Bird Dancer",
  "Abyssal Ancient Giant",
  "Magic Order Guardian",
  "Glancing Hit Crystal",
  "Incarnation of Greed",
  "Elite Magical Archer",
  "Pure Vanilla Cookie",
  "Eunbee and Sapsaree",
  "White King Angelmon",
  "Templar Royal Guard",
...
function extractMonsterNames(compoRaw){
  ...
  while(i<words.length){
    let matched=false;
    // 1. Teste les noms composés connus (liste statique, du plus long au plus court)
    for(const compound of COMPOUND_NAMES){
      const cWords=compound.split(" ");
      const len=cWords.length;
...
const dangerScore=(losses,total)=>{
  if(total<2)return 0;
  return wilson(losses,total)+Math.log10(Math.max(total,1))/3;
};
...
function VDScore({wins,losses,total}){
  ...
  return <span style={{display:"inline-flex",alignItems:"center",gap:3,
    ...
    <span style={{color:T.ink3,fontSize:10}}>/{tot}</span>
  </span>;
}
...
/* ══════════════════════════════════════════════════════════════════════════
   SEARCH WIDGET — Contre-pick principal
   Section 1 : Résultats DIRECTS (indigo)
   Section 2 : Axes de réflexion élémentaires (ambre)
══════════════════════════════════════════════════════════════════════════ */
...
function findElemVariants(scope,directDefs,query){
  if(!MONSTER_CACHE_LOADED||MONSTER_CACHE.size===0)return new Set();
  ...
  if(!directElemsSet.size)return new Set();
  const relevant=new Set(directElemsSet);
  directElemsSet.forEach(e=>{
    if(COUNTER_ELEM[e])relevant.add(COUNTER_ELEM[e]);
    SAME_ELEM_GROUP.forEach(g=>{if(g.includes(e))g.forEach(x=>relevant.add(x));});
  });
  ...
  scope.forEach(d=>{
    ...
    if(commonNames>=1&&defElems.some(e=>relevant.has(e)))variantDefs.add(d.defense);
  });
  return variantDefs;
}

function OffenseCard({r,idx,histWarning,medals,isVariant}){
  const border=isVariant?T.amberMid:idx===0?T.indigoMid:T.line;
  const bg=isVariant?"rgba(245,158,11,0.06)":idx===0?"rgba(99,102,241,0.06)":T.s2;
  const shadow=isVariant?"none":idx===0?T.indigoGlow:"none";
  return(
    <div style={{background:bg,border:`1px solid ${border}`,boxShadow:shadow,
...
function SearchWidget({data,liveGuild}){
  ...
  const allMonsters=useMemo(()=>{
    const freq={};
    data.forEach(d=>{if(!d.defense)return;d.defense.split(" ").forEach(w=>{if(w.length>=3)freq[w]=(freq[w]||0)+1;});});
  ...
  const elemResults=useMemo(()=>{
    ...
    const elems={};
    [...variantDefs].forEach(def=>{def.split(" ").forEach(w=>{const e=monsterElement(w);if(e)elems[e]=(elems[e]||0)+1;});});
    const mainElem=Object.entries(elems).sort((a,b)=>b[1]-a[1])[0]?.[0];
    return results.filter(r=>!directNames.has(r.name)).map(r=>({...r,mainElem}));
  ...
  return <div>
    ...
    {directResults.length>0&&(
      <div style={{marginBottom:16}}>
        ...
        <div style={{display:"grid",
          ...
          gap:8,marginBottom:directResults.slice(3).length?10:0}}>
    ...
    {query.trim()&&!directResults.length&&!elemResults.length&&(
...
function ExpandableDef({s,data,isOpen,onOpen,dimmed}){
  ...
  const dc=s.lossRate>=65?T.red:s.lossRate>=45?T.amber:T.ink2;
...
function WorstDefsCard({worstDefs,data,openDef,setOpenDef,worstN,setWorstN,maxN}){
  ...
  return <Card>
    <SH title={t("worstTitle")} sub={t("worstSub")}
      right={<SliderControl value={worstN} onChange={setWorstN} max={maxN}/>}/>
    {worstDefs.length===0
      ?<Empty>Aucune défense (min 3 attaques)</Empty>
      :<>
        ...
        {worstDefs.length>8&&(
          <button onClick={()=>setShowAll(v=>!v)}
            ...
            {showAll?t("reduce"):`${t("more")} ${worstDefs.length-8} de plus`}
          </button>
        )}
      </>}
  </Card>;
...
function SmartDock({tab,setTab,data,onImport,importMsg,fileRef}){
  ...
  const TABS=[
    {id:"counterpick",label:t("counterpick"),icon:"⚡"},
    {id:"analyse",    label:t("analyse"),    icon:"▤"},
    {id:"guilde",     label:t("guilde"),     icon:"◈"},
    {id:"combat",     label:t("combat"),     icon:"≡"},
  ];
  const opacity=scrolled&&!hovered?0.14:1;
  return <div className="dock-wrap" style={{position:"fixed",top:14,left:"50%",
    transform:"translateX(-50%)",zIndex:1000,opacity,transition:`opacity 0.4s ${EASE}`}}
    ...
    <div style={{display:"flex",alignItems:"center",
      ...
      <div style={{padding:"4px 12px 4px 10px",borderRight:`1px solid ${T.line}`,
      ...
      <span style={{fontSize:11,color:T.ink3,padding:"0 4px",
        fontVariantNumeric:"tabular-nums",whiteSpace:"nowrap"}}>{data.length}</span>
...
export default function App(){
  ...
  useEffect(()=>{
    Promise.all([dbLoad(),loadMonsterCache()])
  ...
  return <div style={{minHeight:"100vh",background:T.bg,color:T.ink1,fontFamily:FONT}}>
    <style>{`
      *{box-sizing:border-box;}
      @media(max-width:700px){
        ...
        .panel-fixed{position:fixed!important;top:0!important;left:0!important;
    ...
    <div style={{maxWidth:1160,width:"100%",margin:"0 auto",
      ...
      {tab==="analyse"    &&<AnalysePage    data={data}/>}
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{background:`linear-gradient(160deg,rgba(99,102,241,0.12) 0%,rgba(99,102,241,0.02) 100%)`,
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18,flexWrap:"wrap",gap:10}}>
        <div>
          <div style={{fontSize:18,fontWeight:800,color:T.ink1,letterSpacing:-0.5}}>{t("counterpick")}</div>
        ...
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          ...
          <div style={{textAlign:"right"}}>
            ...
            <div style={{fontSize:10,color:T.ink3,marginTop:2}}>{totalW}V · {data.length-totalW}D</div>
          </div>
        </div>
      </div>
...
function AnalysePage({data}){
  ...
  const worstDefs=useMemo(()=>
    computeStats(data.slice(-worstN),"defense")
      .filter(s=>s.total>=3)
      .map(s=>({...s,lossRate:Math.round((s.losses/s.total)*100),danger:dangerScore(s.losses,s.total)}))
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div className="grid-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
      <Card>
        <SH title={t("topOffTitle")} sub={t("click_off")}
      ...
      <Card>
        <SH title={t("topDefTitle")} sub={t("click_def")}
    ...
    <WorstDefsCard worstDefs={worstDefs} data={data}
      openDef={openDef} setOpenDef={setOpenDef}
      worstN={worstN} setWorstN={setWorstN} maxN={data.length||2000}/>
  </div>;
}
  ...
  return <Card style={{borderLeft:`2px solid ${T.indigo}`}}>
    ...
    <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
      {[[`${playerWR}%`,"WR",playerWR>=55?T.green:T.red],
        [wins,"V",T.green],[scope.length-wins,"D",T.red],
        [scope.length,"Att",T.ink2]].map(([v,l,c])=>(
      ...
      {worstDefs.slice(0,7).map(d=>(
        <div key={d.name} style={{display:"flex",alignItems:"center",gap:8,
          ...
          <span style={{fontSize:11,color:T.red,fontWeight:700,
            fontVariantNumeric:"tabular-nums",width:36,textAlign:"right"}}>{d.lossRate}%</span>
  ...
  return <Card style={{borderLeft:`2px solid ${T.indigo}`}}>
    ...
    <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
      {[[scope.length,"Att",T.ink2],[wins,"V",T.green],[scope.length-wins,"D",T.red],
        [`${wr(wins,scope.length)}%`,"WR",wr(wins,scope.length)>=50?T.green:T.red]].map(([v,l,c])=>(
        <div key={l} style={{flex:"1 1 55px",background:T.s2,borderRadius:8,padding:"8px 10px"}}>
          <div style={{fontSize:9,color:T.ink3,textTransform:"uppercase",letterSpacing:1,marginBottom:3}}>{l}</div>
          <div style={{fontSize:18,fontWeight:700,color:c,fontVariantNumeric:"tabular-nums",lineHeight:1}}>{v}</div>
        </div>
    ...
    <div style={{fontSize:10,color:T.ink3,textTransform:"uppercase",
      letterSpacing:1,marginBottom:8,fontWeight:600}}>{t("defsUsed")}</div>
...
function Guilde({data}){
  ...
  const guildPerf=useMemo(()=>{
    ...
    data.forEach(d=>{
      ...
      map[g].total++;if(d.victoire){map[g].wins++;map[g].history.push(1);}else map[g].history.push(0);
    });
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:12}}>
    ...
    {view==="membres"&&(
      <div className="grid-2col" style={{display:"grid",
        gridTemplateColumns:selectedPlayer?"minmax(0,1fr) 360px":"1fr",gap:12}}>
        <Card>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              <thead>
                <tr style={{borderBottom:`1px solid ${T.line}`}}>
                  {[["#",""],["Joueur",""],["Att","total"],["WR","wr"],
                    ["V",""],["D",""],["Streak","streak"]].map(([h,s])=>(
                ...
                {sorted.map((p,i)=>{
                  ...
                  return <tr key={p.name}
                    ...
                    <td style={{padding:"8px 10px",color:T.green,
                      fontVariantNumeric:"tabular-nums"}}>{p.wins}</td>
                    <td style={{padding:"8px 10px",color:T.red,
                      fontVariantNumeric:"tabular-nums"}}>{p.total-p.wins}</td>
    ...
    {view==="guildes"&&(
      <div style={{display:"grid",
        gridTemplateColumns:selectedGuild?"minmax(0,1fr) 400px":"1fr",gap:12}}>
        <Card>
          <div style={{marginBottom:12}}>
            <Inp value={guildSearch} onChange={e=>setGuildSearch(e.target.value)}
              placeholder={t("searchGuild")} style={{fontSize:12,padding:"6px 10px"}}/>
          </div>
          <div style={{display:"grid",
              ...
              return <div key={g.name}
                ...
                <div style={{display:"flex",gap:12,marginBottom:8}}>
                  <span style={{fontSize:11,color:T.ink2,fontVariantNumeric:"tabular-nums"}}>
                    <span style={{fontWeight:600}}>{g.total}</span> att.</span>
  ...
  return(
    <div className="combat-grid" style={{display:"grid",
      ...
      {rows.map(d=>{
        ...
        const wc=d.victoire?T.green:T.red;
        return(
          <div key={d.id} onClick={()=>setOpenId(isOpen?null:d.id)}
            ...
            <div style={{display:"flex",alignItems:"center",
              ...
              <span style={{fontSize:10,fontWeight:700,color:wc,
            ...
            {isOpen&&(
              <div style={{marginTop:10,paddingTop:10,
                ...
                <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                  {d.joueurAdverse&&<span style={{fontSize:10,color:T.ink3}}>
                    vs <span style={{color:T.ink2}}>{d.joueurAdverse}</span></span>}
                  {d.guildeAdverse&&<span style={{fontSize:10,color:T.ink3}}>
                    Guilde : <span style={{color:T.ink2}}>{d.guildeAdverse}</span></span>}
                  {d.date&&<span style={{fontSize:10,color:T.ink3}}>{d.date}</span>}
                </div>
...
function DetailCombat({data,setData}){
  ...
  const filtered=useMemo(()=>data.filter(d=>
    (!sess||d.session===sess)&&(!fPl||d.joueur===fPl)&&(!fRes||d.resultat===fRes)
  ),[data,sess,fPl,fRes]);
  ...
  return <Card>
      ...
      {["","Victoire","Défaite"].map(r=>(
        <button key={r} onClick={()=>setFRes(r)}
          style={{padding:"5px 12px",borderRadius:20,
            border:`1px solid ${fRes===r?(r==="Victoire"?T.green:r==="Défaite"?T.red:T.indigo):T.line}`,
    ...
    {showForm&&(
      <div style={{background:T.s2,border:`1px solid ${T.line}`,borderRadius:9,
